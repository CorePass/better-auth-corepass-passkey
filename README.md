...
| **POST** | `/webauthn/restore` | **CorePass** sends Ed448-signed `{ coreId, restoreId, timestamp }`. Verifies signature and revokes old **sessions**. Old **passkeys are kept** until the replacement is registered (see below). |
...
**Restore matches the Core ID case-insensitively (since 0.1.32).** Enrichment always stores `coreId` uppercase, but `corepass_profile.coreId` is a plain unique TEXT column with no `COLLATE NOCASE`, so lookups are case-sensitive. A row written by hand — notably a support flow that switches a user to a new Core ID after document verification — is easily lowercase, the form used almost everywhere else. The miss was silent and total: the restore challenge reverted to `pending`, the error went to CorePass rather than the browser, and the page span until it reported "expired", so no replacement passkey was ever created. Restore now tries uppercase, then the value as given, then lowercase. The default path is unchanged — a canonical uppercase row still matches on the first lookup, and the extra lookups only run where the request was already going to 404.

**Core ID ownership is resolved before `user.email` is written (since 0.1.31).** `user.email` is UNIQUE in the Better Auth schema, so when a Core ID's previous owner still held the address, the enrichment `user` update raised a raw constraint error: the app received a bare 500, the `CORE_ID_TAKEN` path never ran, and the half-registered user and passkey were left orphaned (a passkey row with no `corepass_profile`). The collision check now runs first — reclaiming an orphan also frees the address — and the update itself is guarded, failing with `EMAIL_TAKEN` and full cleanup rather than a 500.

**Revocation is deferred (since 0.1.30).** `POST /webauthn/restore` revokes sessions immediately but leaves existing passkeys in place; they are deleted only after the replacement credential is registered, in the plugin's `/passkey/verify-registration` after-hook. Previously the old passkeys were dropped at signature time, so any failure in the second half of the flow — CorePass declining, a WebAuthn error, the user closing the tab — left the account with **zero** credentials and only another restore as a way back, which revoked again on the next pass. The trade-off is that between the CorePass signature and the new registration the old credential still authenticates; sessions are revoked either way, so other devices lose access immediately.
...
| `allowedAaguids` | `string \| string[] \| false` | Core Pass AAGUID `636f7265-7061-7373-6964-656e74697479` | AAGUID allowlist for passkey registration. Default restricts to Core Pass. Use a string (one), string[] (many), or `false` to allow any authenticator. Enforced in the `/passkey/verify-registration` after-hook: a disallowed credential row is deleted and the request fails with `AAGUID_NOT_ALLOWED`. (Before 0.1.30 this relied on a passkey `create.before` DB hook, which never fired — `@better-auth/passkey` writes the row with `adapter.create()`, bypassing `databaseHooks` — so the allowlist was silently inert.) |
